...
`s` shares the selection up into the global library, or drops the global copies of papers already there and keeps the local ones — the mirror of `m`. Press `t` (or click the `global` badge) to hide the global tier: reads become purely local. No tier gesture ever destroys a sole copy — removing a local paper rescues it into the global library, and un-sharing a paper the local library does not hold is refused.
