...
With the global tier enabled (the default), reads merge both tiers. Writes are local-first: whenever a local library is in scope it is the one an import writes to, the way an install inside a virtualenv lands in the virtualenv rather than system-wide. `--global` (`I` in the TUI) writes both, `--global-only` writes only the global library, and with no local library in scope everything goes to the global one unflagged.
`s` shares the selection up into the global library, or drops the global copies of papers already there and keeps the local ones — the mirror of `m`. A TUI session started where a local library is in scope opens on that library alone; press `t` (or click the `global` badge) to show the global tier, and again to hide it. The CLI is unaffected — `list` and `search` merge both tiers unless you pass `--no-global`. Whether a cite resolves is never a question about the tier you are looking at: the manuscript page reads both, so a paper in your global library shows as `library`, never as missing. No tier gesture ever destroys a sole copy — removing a local paper rescues it into the global library, and un-sharing a paper the local library does not hold is refused.
...
### Keys
- `/` — live filter of the scope you are on, each scope keeping its own (query language below); `S` — new ADS query (`↑`/`↓` sets the result count, `⌃r` opens the menu of what ADS returns — both also clickable; a DOI, an arXiv link, or the journal's own article URL finds that paper instead of searching for the text)
...
---
## CLI
`list`, `search [--ads]`, `add <bibcode|DOI|article URL> [--global]`, `show <key>`, `rm <key> [--local-only]` (sole copies rescued), `import <file.bib> [--global|--global-only|--local-only] [--cited-only] [--rename-citekeys] [--dry-run]`, `refs [FILE] [--prune|--no-sync|--check] [--dry-run]`, `tidy [--dry-run]`, `convert bibcode|full|short` (uniform cite keys, rewritten in your `.tex` and `.md` sources alike), `update [--all]` (arXiv → published refresh, same key forever), `config [ads_token|email <value>]` (show or set the environment, including how much of the day's ADS allowance the token has spent), `gc` (report what the machine-local caches cost), plus `--library PATH` (relocate the global tier) and `--no-global`.
