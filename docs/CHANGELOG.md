...
- `astrobib config` reports how many stored metrics belong to papers outside this library, and `astrobib config --prune-metrics` drops them, listing each. Pruning is never automatic: the metrics store is shared across libraries, so only you know whether an unmatched key belongs to a library that simply is not mounted right now.
