...
### Added
- Mode changes say what they did, in the log as well as on screen: the metric cycle, every sort, and every column shown, hidden or resized. Showing or hiding a view is not among them — the view appearing is its own confirmation, and logging it would only push out the messages that are there because you would otherwise have missed them. Repeatable controls supersede their own last line rather than appending — holding `←` on a column width leaves one entry reading `authors column 19 wide`, not eight — while anything you did not directly cause (a download finishing, a query landing) always appends.
- `E` edits a saved query in place — its text, its result limit and what ADS returns, all in the prompt at once. `S` still always composes a new query, so a saved one's text used to be write-once: changing it meant replacing the tab and losing its name, its display sort and its cached results with it. An unnamed tab's label follows the edited text; a name you typed outlives it.
- The footer offers what the scope you are on is for, where it would otherwise be blank: `edit query (E)` on a query, `new ADS query (S)` on the library or manuscript. A prompt or a fresh message outranks it, since those are there because you would have missed them and this is only there because the line was going to be empty.
- `]` off the rightmost capsule composes a new query rather than wrapping round to the library — the strip already ends with `+ new`, so stepping right reaches it.
- `N` names a saved query. The capsule is labelled from the query text otherwise, which says what you typed rather than what you were doing — `LRD reading` beats `abs:"little red dot"`. A name outlives edits to the query, since re-deriving would quietly discard a decision; an empty name restores the derived one. The confirmation lands on the footer *after* the prompt has closed, because the prompt occupies the footer and a note raised while it is up would be drawn over and never seen.
- The query prompt reads as one phrase — `ADS returns 20 (↑↓) most cited (⌃r)`. Only the mode is a control: the limit is stepped with keys no pointer can press, so only the mode highlights under the pointer and answers a click.
- The query prompt shows what ADS will return — `ADS returns newest posting (⌃r)` — cycled with that chord or by clicking it, which underlines under the pointer. It is named in full rather than reduced to a symbol, because the name changing *is* the feedback that the mode changed. Previously this could only be reached after the tab existed, so a query's first run could never be anything but the default — so a query can be composed as a feed of postings, of publications, or by citations *before* it is first run. Previously the selection sort was only reachable from the table panel, after the tab already existed, so a first run could never be anything but the default. In the prompt it sits beside the result limit because the two are the same kind of thing: properties of the query that are not query *syntax*, riding alongside `q` as the `sort` and `rows` API parameters. Both are drawn in their own colour and the text cursor never enters them, since typing there would not reach ADS. The choice is deliberately not persisted — it is made when a query is composed and matters while it is being worked, so a restored tab starts from the default rather than from a choice made days ago.
- Saved queries read as a feed of postings. ADS is now asked for records by `entry_date` — when it indexed the record — rather than by publication date, so "the newest 20 matching this" means the newest 20 *postings*; a 2019 paper published this month no longer outranks yesterday's preprint. A new `Entered` column shows that date beside `Year`, and sorts on it. The two are separate mechanisms and never interchangeable: the ADS sort decides which records come back, the column sort reorders the ones already in hand, so ranking a feed by citations gives the most cited among the newest 20, not the most cited overall. Each query tab stores its own selection sort, changed from the table panel; single-record lookups still use publication date.
- A table panel (`|`, or the `table` footer badge) chooses which columns each view shows, how wide they are (`←`/`→`), and what it sorts by — including columns that are *not* shown, so a query can be ordered by entry date without spending ten columns of screen on the dates. It is a toggle view beside the table rather than a modal, so it takes the arrow keys while open; `Esc` hands them back without closing it. Untouched columns keep the responsive behaviour they always had — the author column still scales with the terminal, `Key` still drops first when things get tight, the metric swatch is still off — so a configuration you have not edited is indistinguishable from none. Whichever column is absorbing the leftover width shows `—` instead of a size; hide it and the job passes to the next candidate, so no column is unhidable.
- `Tab` toggles which of the two arrow-navigable areas the arrows drive — the table and the table panel — so the panel no longer needs `Esc` to give them back (`Esc` still works, and neither closes the panel).
- Column headers say what they are on rollover, so the one-glyph indicator columns are no longer a guess: `●` reads "in the manuscript" in the library and "already in your library" on a query page, since it means different things in the two places.
...
### Fixed
- Opening the event log or the keys sheet no longer shortens the pub card. Both used to span the whole frame, so showing either stole rows from everything — including a card that has nothing to do with them. They stack inside the table's column now, where they belong; the card and the table panel keep their full height.
- The pub card's view toggler could be drawn over by the card's own content — a long keyword line or the "add to manuscript" chip landing on the last row. The toggler is pinned there, and the body was allowed to reach it. It was easy to miss while the log and keys panes were shortening the card; now that they do not, the body stops one row above the toggler.
- The table cursor could point past the end of a query or manuscript scope. A refresh that returns fewer records than the tab was holding left the selection where it was, so the pub card rendered blank and every row action had no target. The library had `refilter` for this and the other scopes had nothing; the cursor is now clamped to a row that exists wherever it is drawn.
- The table panel scrolls. Its list could be taller than the pane — a query scope offers eight columns plus the four ADS options — and it simply stopped drawing at the bottom edge, so the cursor kept descending into rows that were never rendered and appeared to vanish below the last one. The `ADS returns` heading disappeared the same way.
...
- Every control in the table panel is clickable where it looks clickable, and only there. Sorting is the marker cell at the right of a row — clicking a row elsewhere just selects it — and hovering that cell previews, dimmed, the arrow a click would leave behind, so a column that is not the sort column still advertises the control. The `›` widen target had been sitting two columns left of the `›` itself, so clicking it did nothing while clicking the width readout widened.
- `←`/`→` on a column with no settable width does nothing, quietly. It used to report every such case as "a hidden column has no width", which was wrong for the locked one-cell columns and for whichever column is absorbing the leftover space; the panel draws those rows without the `‹ ›` nudges, so there is no affordance to refuse and nothing worth saying.
