...
### Added
- Changing what ADS returns says so in words, not only by changing its glyph, and the prompt grows an echo line above itself to carry it — the prompt occupies the footer the note would otherwise have used, minibuffer-style, so feedback for anything done *while* composing had nowhere to land.
- The query prompt shows what ADS will return, as one glyph — `⇓` newest posting, `↓` newest published, `≫` most cited, `≈` most relevant — cycled with `⌃r` or by clicking it. Rolling it names the mode and the chord in place of the standing hint. Previously this could only be reached after the tab existed, so a query's first run could never be anything but the default — so a query can be composed as a feed of postings, of publications, or by citations *before* it is first run. Previously the selection sort was only reachable from the table panel, after the tab already existed, so a first run could never be anything but the default. In the prompt it sits beside the result limit because the two are the same kind of thing: properties of the query that are not query *syntax*, riding alongside `q` as the `sort` and `rows` API parameters. Both are drawn in their own colour and the text cursor never enters them, since typing there would not reach ADS. The choice is deliberately not persisted — it is made when a query is composed and matters while it is being worked, so a restored tab starts from the default rather than from a choice made days ago.
