...
### Added
- Mode changes say what they did, in the log as well as on screen: the four view toggles, the global tier, the verbatim-bib view, the metric cycle, every sort, and every column shown, hidden or resized. Repeatable controls supersede their own last line rather than appending — holding `←` on a column width leaves one entry reading `authors column 19 wide`, not eight — while anything you did not directly cause (a download finishing, a query landing) always appends.
- `E`, or `edit query (E)` in the footer of a query page, edits a saved query in place — its text, its result limit and what ADS returns, all in the prompt at once. `S` still always composes a new query, so a saved one's text used to be write-once: changing it meant replacing the tab and losing its name, its display sort and its cached results with it. An unnamed tab's label follows the edited text; a name you typed outlives it.
