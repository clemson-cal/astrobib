...
### Added
- The `S` prompt shows what ADS will select by, and `⇥` cycles it — so a query can be composed as a feed of postings, of publications, or by citations *before* it is first run. Previously the selection sort was only reachable from the table panel, after the tab already existed, so a first run could never be anything but the default. It sits beside the result limit because the two are the same kind of thing: properties of the query that are not query *syntax*, riding alongside `q` as the `sort` and `rows` API parameters. Both are drawn in their own colour and the text cursor never enters them, since typing there would not reach ADS.
