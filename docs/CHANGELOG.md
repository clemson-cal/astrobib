...
- `astrobib gc` reclaims machine-local caches — orphaned PDFs, stale query results, metrics for papers outside this library — and never touches a `.bib` file. It reports by default, naming what it would delete with sizes; `--clean` frees the disposable caches, and `--clean --metrics` additionally drops orphaned metrics, which are hand-curated and unrecoverable. Nothing is reclaimed automatically: these caches are shared across every library you open, so a paper living in another library counts as an orphan here. `astrobib config` just points at it.
