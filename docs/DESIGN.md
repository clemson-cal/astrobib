...
**`import` should take a `--dry-run`, and does not yet.** It was left out on the argument that the map is a product of the import — an entry's canonical key comes from the record ADS resolved it to — so there is nothing to preview. That argument is wrong, and in a way worth recording: resolution is a *read*. Only `save_entry` writes, so a dry run can resolve every entry, compute the whole map, and report both halves of what it would do — which `.bib` files it would write to which tier, and which cites it would rewrite in which sources — while touching nothing. That is the run a coauthor's export deserves before it is let near either.
...
The flag refuses outside a manuscript with sources, and refuses *before* importing anything: discovering there was nowhere to apply the rewrite after the fact would leave the map on the terminal and the library changed, which is the state the flag exists to end.
...
**`convert` is only half markdown-capable, and should be whole.** It shares the rewriter, so it now writes markdown sources correctly, but it still *scans* through `manuscript_tex_files`: in a markdown-only manuscript it exits with "No .tex sources found", and in a mixed one it rewrites only the keys its `.tex` files happened to cite. The scan has to become `manuscript_source_files` with `scan_md_files` beside `scan_tex_files`. The one piece of real design in it is the wikilink rule — a `[[Key]]` is a citation only where it resolves, so unresolved ones must be dropped before the map is built rather than being reported as "unresolved (left alone)", which would turn every ordinary note link in the manuscript into a line of output.
