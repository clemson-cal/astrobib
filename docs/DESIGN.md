...
**The re-key map is printed and then thrown away; a command should apply it.** Import of a Zotero or Overleaf export re-keys nearly every entry — `bartos_rapid_2017 → Bartos2016`, `dorazio_accretion_2013 → DOrazio2012` — and the manuscript still cites the old strings. Today `import` prints the map under "Re-keyed (old cites resolve by prefix or bibcode; others show as missing)" and exits; the honest reading of that caveat is that the ones which *don't* resolve are silently broken cites, and the only record of how to fix them is terminal scrollback. A CLI command should rewrite them in the sources.

It cannot be `convert` as it stands. `convert --to short|full|bibcode` already rewrites cites in place (`rewrite_citations` → `convert_citations`, which touches only the arguments of cite commands) and regenerates `refs.bib` after, and that machinery is what a re-key command should reuse. But `convert` derives its map from `resolve_citation`, which knows full keys, unambiguous prefixes and bibcodes — a foreign key like `krauth_disappearing_2023` resolves to nothing at all, so the mapping is unrecoverable after the import that computed it. That leaves two shapes: apply the map during import behind a flag, or persist it so a later command can. The second is the more useful one — an import into a coauthor's tree is often reviewed before the sources are touched — but it puts a rename log somewhere, and per "What the database is and is not" that log is user-local state, never a file in `bib/`.

Two constraints on whichever shape wins. Rewriting someone's manuscript is the one destructive thing astrobib does to files it does not own, so it takes a `--dry-run` as `convert` does, and it prints what it changed per file. And it must cover `.md` sources: cite scanning handles pandoc and wikilink citations everywhere else, but `manuscript_tex_files` — what `convert` walks — collects `.tex` only, so a markdown manuscript would be re-keyed in its bibliography and nowhere in its prose.
