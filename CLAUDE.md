...
- **`m` now needs a keystroke that the default view does not give you.** A session in a project opens with the global tier hidden, and `m` (manuscript ±) is unavailable while it is — correctly, since every visible paper is then already a member and ± could only mean "remove", which is why the message names `t`. But populating a fresh project's db from your library is now `t` then `m`, where it used to be `m`. Left alone deliberately: the alternatives (making `m` mean "add" while the tier is hidden, or auto-showing the tier for the gesture) both make one key mean two things depending on state. Revisit if the two-step turns out to be the common path rather than the occasional one.
- **`s` means share on the table and sort inside the `|` panel.** No mechanical conflict — the panel takes focus when it opens, so `columns_panel_key` claims `s` before the table sees it — but they are two meanings for one letter. Nothing to fix unless the collision actually bites; the alternative is renaming one of two established gestures.
- **`refilter` rebuilds a `SearchDoc` per ADS row on every keystroke.** A query page's rows are not library entries, so they have no cached lowercased fields the way `Entry::search_doc` does: each keystroke lowercases every visible article's title and abstract (`src/tui/rows.rs`). At ADS's default result counts this is well under a millisecond, and caching would need invalidating whenever a tab's articles change. Measure before fixing.
