...
- **Twenty of `App`'s 91 fields are one thing: a per-frame hit-test registry.** Nineteen rect caches whose shared invariant — a rect must not outlive the frame that drew it — is maintained by hand in three overlapping mechanisms, and which is why `on_click` is 322 lines. Full analysis and a staged plan, verification first, in [docs/plans/hit-registry.md](docs/plans/hit-registry.md). Deferred for budget, not for doubt; it is behaviour-bearing, so it needs click scenarios written *before* anything moves.
