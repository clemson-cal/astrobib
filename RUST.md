...
- Selection mode (iOS-style, replaces the Python TUI's check-marks): Space or a click in the leftmost gutter enters the mode and toggles rows (◯ unselected, ◉ selected); Esc exits and clears; `s` stars/unstars the whole selection (any-unstarred → star all). Mouse support includes click-to-highlight and wheel scrolling.
