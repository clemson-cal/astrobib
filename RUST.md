...
- Pub card PDF buttons (act on the card's entry): `arXiv ↓  OA ↓  browser ↓  pick …` when uncached (each dimmed if its prerequisite is missing), `open  clear` when cached, `cancel` while a browser watch is pending. `pick …` opens a modal ~/Downloads PDF list (newest first, ⏎ imports a copy after a %PDF header check). Pub card toggle is `D`/`z`.
