...
- Actions panel (`ctrl+p`, rightmost): every action with its key (all single-char: ␣ ⌫ …), unavailable ones dimmed per a single-vs-multi policy (browser DL and pick are single-target; download/open/clear dim when no target qualifies; manuscript dims without an active db). Rows are clickable; the same dispatcher backs keys, panel clicks, card buttons, and footer badges. The `y` chord replaces the panel with the copy menu while active (no tab bar), restoring after.
