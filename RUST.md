...
- Actions block (`ctrl+p` or the keys badge): a titleless two-column list at the bottom of the pub card, as tall as it needs, every action with its single-char key (␣ ⌫ …), unavailable ones dimmed per a single-vs-multi policy (browser DL and pick are single-target; download/open/clear dim when no target qualifies; manuscript dims without an active db). Items are clickable; the same dispatcher backs keys, item clicks, card buttons, and footer badges. Showing it implies the card. The `y` chord replaces the block with the copy menu while active, restoring both view states after.
...
- Yank-cells (⧉) beside the card's title, abstract, and cite key copy that datum directly; the abstract yank copies the full stored text — the card's 1000-char/height truncation is display-only, matching Python.
