...
- Selection mode (iOS-style, replaces the Python TUI's check-marks): Space, a click in the leftmost gutter, or cmd/option/ctrl+click anywhere on a row enters the mode and toggles rows (◯ unselected, ◉ selected); Esc exits and clears, and so does toggling the last selected row off. Mouse support includes click-to-highlight and wheel scrolling.
